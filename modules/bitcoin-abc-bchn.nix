...
in {
  config.kubernetes.moduleDefinitions.bitcoin-abc-bchn.module = {name, config, ...}: let
    bitcoinAbcBchnConfig = ''
      ##
  ...
  in {
    options = {
      image = mkOption {
        description = "Name of the bitcoin-abc-bchn image to use";
        type = types.str;
      };

      replicas = mkOption {
        description = "Number of bitcoin-abc-bchn replicas";
        type = types.int;
        default = 1;
      };
    ...
    config = {
      kubernetes.resources.statefulSets.bitcoin-abc-bchn = {
        metadata.name = name;
        ...
        spec = {
          ...
          template = {
            ...
            spec = {
              ...
              initContainers = [{
                name = "copy-bitcoin-abc-bchn-config";
                image = "busybox";
      ...
      kubernetes.resources.configMaps.bitcoin-abc-bchn = {
        metadata.name = "${name}-config";
        data."bitcoin.conf" = bitcoinAbcBchnConfig;
      };

      kubernetes.resources.services.bitcoin-abc-bchn = {
        metadata.name = name;
        metadata.labels.app = name;
        spec = {
          selector.app = name;
